...
async def financial_diagnosis(
    ...
    for r in results:
        month_label = r["month"]
        table_lines.append(f"### 📅 {month_label}")  # 월별 구분 제목 추가
        table_lines += [
...
[월별 주요 지표 테이블 — 각 달별로 구분됨]
(아래 표들은 {start_month}~{end_month} 기간의 월별 데이터를 포함)
...
⚠️ 반드시 아래 표를 그대로 Markdown 테이블 형식(|---|---|---|---|---|)으로 출력할 것.  
테이블은 절대 생략하거나 요약하지 말고 그대로 포함하라.
[추가 지시]
