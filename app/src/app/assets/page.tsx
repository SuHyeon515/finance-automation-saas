...
const loadLiquidAssets = async () => {
  ...
  const loadAssets = async () => {
    try {
      const res = await fetch(`${API_BASE}/assets_log?branch=${encodeURIComponent(branch)}`, { credentials: 'include' })
      const json = await res.json()
      setAssetLogs(json.items || [])
  ...
  const deleteAsset = async (id: string) => {
    ...
    const res = await fetch(`${API_BASE}/assets_log/${id}`, { method: 'DELETE', credentials: 'include' })
